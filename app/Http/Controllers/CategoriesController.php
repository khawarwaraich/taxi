...
class CategoriesController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['categories'] = DB::table($outlet_id . '_categories')->get();
    ...
    {
        $outlet_id = Session::get('outlet_id');
        $categories_rank = DB::table($outlet_id . '_categories')
                                        ->select('rank')
                                        ->get();
        for ($i = 1; $i <= 30; $i++) {
            $resultRank[$i] = $i;
    ...
    {
        $outlet_id = Session::get('outlet_id');
        $categories_data = DB::table($outlet_id . '_categories')->where('id', $id)->get();
        $data['categories_data'] = $categories_data[0];
        for ($i = 1; $i <= 30; $i++) {
            $resultRank[$i] = $i;
        }
        $data['rank'] = $resultRank;
    ...
    public function store(Request $request)
    {
        ...
        if (isset($update_id) && !empty($update_id) && $update_id != 0) {
            $categories = DB::table($outlet_id . '_categories')->where('id', $update_id)->update($data);
            $categories_data = DB::table($outlet_id . '_categories')->where('id', $update_id)->get();
            ...
            if ($_FILES['image']['size'] > 0) {
                ...
                if (isset($categories_data->image) && !empty($categories_data->image)) {
                    ...
                    upload_images(ORIGNAL_IMAGE_PATH_CATEGORIES, LARGE_IMAGE_PATH_CATEGORIES, MEDIUM_IMAGE_PATH_CATEGORIES, SMALL_IMAGE_PATH_CATEGORIES, $file, $categories_data->id, 'image', $outlet_id . '_categories');
                } else {
                    upload_images(ORIGNAL_IMAGE_PATH_CATEGORIES, LARGE_IMAGE_PATH_CATEGORIES, MEDIUM_IMAGE_PATH_CATEGORIES, SMALL_IMAGE_PATH_CATEGORIES, $file, $categories_data->id, 'image', $outlet_id . '_categories');
                }
            }
            ...
            $check_category = DB::table($outlet_id . '_categories')->where('cat_name', $data['cat_name'])->get();
            if (isset($check_category[0]) && !empty($check_category[0]->id)) {
                return redirect()->route('admin:categories')->with('error', 'Category already exists!');
            } else {
                $categories = DB::table($outlet_id . '_categories')->insert($data);
                $last_in_id = DB::getPdo()->lastInsertId();
    ...
    public function change_status(Request $request)
    {
        ...
        if (isset($cat_id) && !empty($cat_id)) {
            ...
            $categories = DB::table($outlet_id . '_categories')->where('id', $cat_id)->update(['status' => $cat_status]);
            $status = true;
    ...
    {
        $outlet_id = Session::get('outlet_id');
        $categories = DB::table($outlet_id . '_categories')->where('id', $id)->delete();
        if ($categories == 1) {
